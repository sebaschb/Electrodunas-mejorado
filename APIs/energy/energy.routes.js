...
const router = Router();
const redis = new Redis(process.env.REDIS_URL || 'redis://redis_container:6379');
...
router.get("/users/:id/energy", async (req, res) => {
  try {
    ...
    // Almacenar los resultados en caché en Redis
    await redis.set(cacheKey, JSON.stringify(groupedData), "EX", 3600); // 1 hora de expiración
